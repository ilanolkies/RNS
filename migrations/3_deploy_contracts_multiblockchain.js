...
const MultiChainResolver = artifacts.require('./MultiChainResolver.sol');
...
function deployDev(deployer, network, accounts) {
  ...
  return deployer.deploy(ERC677, OWNER, TOTAL_SUPPLY, opts)
    ...
    .then(() => {
      return deployer.deploy(MultiChainResolver, RNS.address, PublicResolver.address, opts);
    })
    .then(() => {
      return RNS.at(RNS.address).setDefaultResolver(MultiChainResolver.address, opts);
    })
    .then(() => {
...
function deploy(deployer, network, accounts) {
  ...
  return deployer.deploy(RNS)
    ...
    .then(() => {
      return RNS.at(RNS.address).setDefaultResolver(MultiChainResolver.address);
    })
    .then(() => {
      return deployer.deploy(SafeMath);
    })
